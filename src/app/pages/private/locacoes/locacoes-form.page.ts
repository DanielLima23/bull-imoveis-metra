...
export class LocacoesFormPage implements OnInit {
    ...
    this.leaseApi.getById(id).subscribe({
      next: (item) => {
        ...
        this.form.patchValue({
          ...
          cleaningIncluded: !!item.cleaningIncluded,
          optionalContactPartyId: '',
  ...
  submit(): void {
    ...
    const basePayload = {
      ...
      cleaningIncluded: payload.cleaningIncluded,
  ...
  optionalContactSnapshotSummary(): string {
    return [this.form.controls.optionalContactName.value, this.form.controls.optionalContactPhone.value]
